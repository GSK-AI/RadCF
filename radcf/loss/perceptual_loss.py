"""
Unless otherwise stated, this file is a modified version of https://github.com/bytedance/1d-tokenizer/blob/main/modeling/modules/perceptual_loss.py
See ./LICENSE_originals/LICENSE-1d-tokenizer for the original license.
...
class PerceptualLoss(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        # Always in eval mode.
        self.eval()
        # LPIPS / ConvNeXt expect 3-channel input; randomly pick 3 channels per iteration.
        if input.shape[1] > 3:
            idx = torch.randperm(input.shape[1], device=input.device)[:3].sort().values
            input = input[:, idx]
            target = target[:, idx]
        loss = 0.
        num_losses = 0.
